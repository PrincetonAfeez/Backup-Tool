...
2. Move `[Unreleased]` entries in `CHANGELOG.md` under a new `[X.Y.Z] - YYYY-MM-DD` section.
3. Run `pip install -e .` (or rebuild your environment).
4. Confirm `backup-tool version` matches.
5. Run `pytest`, `ruff check backup_tool tests`, and `mypy backup_tool`.
